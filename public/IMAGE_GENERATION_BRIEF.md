...
**App placement:** `src/data/productImageData.ts` (mainImage), `src/catalog/lovableCatalog.ts` (heroImage), product cards, `/collection` page.

---

## CATEGORY 2: Color Swatch Variants (1024x1024)
...
**App placement:** `src/data/productColorImages.ts` -- swatch key → image mapping.
...
**App placement:** `src/data/productImageData.ts` (galleryImages), `src/catalog/lovableCatalog.ts` (gallery).
...
**App placement:** `src/data/siteImageManifest.ts` -- lifestyle categories. Used by `getLifestyleImagesForCarousel()` and `LifestyleGallery`.
...
**App placement:** `src/data/siteImageManifest.ts` (gift-campaign). Used by `GiftOfComfort` component.
...
**App placement:** `src/data/siteImageManifest.ts` (detail-macro). Product detail pages.

---

## CATEGORY 7: Brand and Trust Images (2752x1536)

| # | Filename | Purpose | Prompt | Where It Goes |
|---|----------|---------|--------|---------------|
| 1 | `brand-workshop.webp` | Our Story | Close-up of craftsman's hands stitching leather on a recliner frame. Egyptian workshop, warm tungsten lighting, wood shavings, quality tools. | `/our-story` heritage section |
| 2 | `brand-warehouse.webp` | Trust/Quality | Dandle warehouse: rows of wrapped recliners ready for delivery, clean organized space, Egyptian workers in branded uniforms. | Trust bar, delivery page |
| 3 | `brand-delivery-crew.webp` | Delivery | Two Egyptian delivery men in clean uniforms carrying a wrapped recliner into a Cairo apartment building. Professional, careful handling. | `/trust/delivery` page |
| 4 | `brand-showroom.webp` | Showroom | Modern furniture showroom in Cairo/Giza, multiple Dandle recliners displayed elegantly, warm lighting, minimal signage. | Partners section |
| 5 | `brand-fabric-swatches.webp` | Product pages | Overhead flat-lay of 8-10 fabric swatches fanned out on marble surface. Rich textures: leather, velvet, chenille, linen. | Fabric section |
| 6 | `brand-qc-inspection.webp` | Quality/Trust | Egyptian quality inspector examining recliner mechanism with precision tools. Clean factory setting, white coat. | Warranty page |
| 7 | `hero-og-share.webp` | Social sharing | RelaxMax in warm Zamalek setting, "Dandle - Earned Luxury" text space. 1200x630 for Open Graph. | `<meta og:image>` |
| 8 | `lifestyle-ramadan.webp` | Seasonal | CozyCompanion loveseat in warm living room, iftar table glimpse, lantern light (fanous). No people. | Seasonal campaign |
| 9 | `lifestyle-mothers-day.webp` | Seasonal | RelaxMax with single rose on armrest, soft morning light, gift-wrapped box nearby. No people. | Seasonal campaign |

**Note:** Category 7 images need manual code wiring after upload.

---

## Summary: 78 Total Images

| Category | Count | Dimensions |
|----------|-------|-----------|
| Product Hero Cards | 10 | 1024x1024 |
| Color Swatch Variants | 29 | 1024x1024 |
| Product Gallery | 12 | 2752x1536 |
| Lifestyle Scenes | 10 | 2752x1536 |
| Gift Campaign | 4 | 2752x1536 |
| Detail/Macro | 4 | 2752x1536 |
| Brand/Trust (new) | 9 | 2752x1536 |
...
3. POST to https://rbvbrxjnhmgrtxvwusxr.supabase.co/functions/v1/manage-images
   Body: { "filename": "exact-name.webp", "base64": "...", "contentType": "image/webp" }
4. GET same URL to verify
5. Live at: https://rbvbrxjnhmgrtxvwusxr.supabase.co/storage/v1/object/public/product-images/images/exact-name.webp
```
